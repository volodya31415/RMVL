...
#' L<-list()
#' L[["x"]]<-mvl_write_object(M3, data.frame(x=1:1e6, y=rnorm(1e6)))
...
mvl_read_metadata<-function(MVLHANDLE, metadata_offset) {
	metadata<-mvl_read_object(MVLHANDLE, metadata_offset, recurse=TRUE)
	if(!is.null(metadata)) {
...
#' @param a vector of MVL_OFFSET objects, one per tag, created by \code{mvl_write_object}
#'
#' @export
...
`[.MVL`<-function(MVLHANDLE, y, sql=FALSE, raw=FALSE) {
	...
	if(is.character(y)) {
		if(sql) {
			...
			warning("Not implemented, tinkering in progress")
...
#' Print MVL directory
#' 
#' @param MVLHANDLE handle to MVL file as created by mvl_open()
